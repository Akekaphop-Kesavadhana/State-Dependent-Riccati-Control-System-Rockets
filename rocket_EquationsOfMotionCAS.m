...
% ixy = 0; % Cruciform, axes symmetric rocket ONLY
% iyz = 0; % Cruciform, axes symmetric rocket ONLY
% ixz = 0; % Cruciform, axes symmetric rocket ONLY
syms ixx iyy izz ixz P Q R L M N phi theta psi
ixy = 0; % Cruciform, axes symmetric rocket ONLY
iyz = 0; % Cruciform, axes symmetric rocket ONLY
ixz = 0; % Cruciform, axes symmetric rocket ONLY
MomentOfInertia     = [ixx -ixy -ixz; -ixy  iyy -iyz; -ixz -iyz  izz];
MomentOfInertia_inv = inv(MomentOfInertia)
Hx                  = P*ixx - Q*ixy - R*ixz;
Hy                  = Q*iyy - R*iyz - P*ixy;
Hz                  = R*izz - P*ixz - Q*iyz;
AngularInertialTerm = [Q*Hz - R*Hy; R*Hx - P*Hz; P*Hy - Q*Hx];
ExternalMoment      = [L; M; N];
% A1        = L - Q*R*izz + P*Q*ixz + (Q)^2*iyz + Q*R*iyy - (R)^2*iyz - P*R*ixy; % Assuming ixy=0,iyz=0,ixz=0
% A2        = M - P*R*ixx + Q*R*ixy + (R)^2*ixz + P*R*izz - (P)^2*ixz - P*Q*iyz; % Assuming ixy=0,iyz=0,ixz=0
% A3        = N - P*Q*iyy + P*R*iyz + (P)^2*ixy + P*Q*ixx - (Q)^2*ixy - Q*R*ixz; % Assuming ixy=0,iyz=0,ixz=0
% A         = [A1; A2; A3];
J           = ExternalMoment - AngularInertialTerm;
omega_dot   = MomentOfInertia_inv*J

eulerRateMatrix     =  [1      0      -sin(theta);
                        0   cos(phi)   cos(theta)*sin(phi);
                        0  -sin(phi)   cos(theta)*cos(phi)]
inv_eulerRateMatrix = inv(eulerRateMatrix)
